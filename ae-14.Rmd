...
```{r prop-null-viz}
obs_stat_2 <- manhattan %>%
  filter(over2500 == "greater") %>%
  nrow() / nrow(manhattan)

visualize(null_dist_3) +
  shade_p_value(obs_stat = obs_stat_2, direction = "greater") +
  labs(x = "Sample Probability", y = "Count")
```
...
```{r prop-calc-p-value}
null_dist_3 %>%
  get_p_value(obs_stat = obs_stat_2, direction = "greater")
```
